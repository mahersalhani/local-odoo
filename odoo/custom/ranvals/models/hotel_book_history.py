...
class HotelBookHistory(models.Model):
    ...
    def create(self, vals):
        ...
        # Set room status to 'booked' for all selected rooms
    ...
    def _check_availability(self):
        for room in self.room_ids:
            # Check if there are any bookings for this room between the check-in and check-out dates
            overlapping_bookings = self.env['hotel.book.history'].search([
                ('room_ids', '=', room.id),
                ('state', 'in', ['booked', 'checked_in']),
                '|', '|',
                ('check_in', '<=', self.check_in),
            ...
            if overlapping_bookings:
                raise ValidationError(_("Room %s is not available for the selected dates.") % room.name)
