...
class HotelRoom(models.Model):
    ...
    # Define a method to check room availability between two dates
    @api.model
    def get_available_rooms(self, check_in, check_out):
        rooms = self.search([])
        available_rooms = rooms.filtered(lambda room: not room.booking_ids.filtered(
            lambda b: b.state in ['booked', 'checked_in'] and (
    ...
    def action_view_reservations(self):
        self.ensure_one()
        action = self.env.ref('ranvals.action_hotel_book_history_all').read()[0]
